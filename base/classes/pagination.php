...
class Pagination
{
	...
	*/
	function __construct($max, $total, $max_items = 10, $parameter = 'p', $path) {
		$this->max	  = $max;
		$this->total	= $total;
		...
		if(is_string($this->parameter)) {
			$string = (!empty( $_GET[$this->parameter] )  && ( $_GET[$this->parameter] <= $this->pages()) ) ? $_GET[$this->parameter] : 1;
		}

		# check if the get parameter value is not empty or not higher than the total number of pages.
		$this->get = (is_numeric($this->parameter)) ? $this->parameter : $string;
	}
	...
	*/
	function first($html, $html2='') {
		...
		$r = str_replace('{path}', $this->path, $r);
